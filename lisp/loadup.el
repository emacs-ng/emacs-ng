...
(if dump-mode
    (let ((output (cond ((equal dump-mode "pdump") "remacs.pdmp")
                        ((equal dump-mode "dump") "remacs")
                        ((equal dump-mode "bootstrap") "remacs")
      ...
      (if (not (or (eq system-type 'ms-dos)
          ...
          (let ((name (format "remacs-%s.%d" emacs-version emacs-build-number))
            ...
            (when (equal dump-mode "pdump")
              ...
              (add-name-to-file (expand-file-name "remacs.pdmp"
                                                  invocation-directory)
